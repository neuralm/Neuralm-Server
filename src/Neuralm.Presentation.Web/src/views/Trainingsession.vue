      ...
      <v-col cols="12" sm="8" md="8">
        <v-card class="mx-auto">
          ...
          <v-card-actions>
            ...
            <v-btn class="trainOrganisms" v-if="trainingSession.endedTimestamp === '0001-01-01T00:00:00' && organisms.length > 0 && !tested" @click="trainOrganisms(trainingSession.id)">Auto Train Organisms XOR</v-btn>
          </v-card-actions>
          <v-divider></v-divider>
...
export default class TrainingSessionView extends Vue {
  ...
  public async trainOrganisms(trainingSessionId: string): Promise<void> {
    const xor: Xor = new Xor();
    const trainingSessionState: ITrainingSessionState = (this.$store as Store<IRootState>).state.trainingSession as ITrainingSessionState;

    while (this.$data.counter < 10000) {
      let newOrganisms: Organism[] = [];
      try {
        newOrganisms = this.observerOrganismsToOrganisms(trainingSessionState.organisms);
      } catch (error) {
        this.$snotify.error('Faulty network structure, failed to construct network!');
        return;
      }
      ...
      .then((response: PostOrganismsScoreResponse) => {
        // this.$snotify.success(response.message);
        this.getOrganisms(trainingSessionId);
      })
      .catch((value: PostOrganismsScoreResponse) => {
        this.$snotify.error(value.message);
        console.log(value);
